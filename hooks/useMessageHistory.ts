...
export default function useMessageHistory() {
    ...
    useEffect(() => {
            ...
            try {
                ...
                const threadsWithUsers = await Promise.all(
                    threadsData.map(async (thread) => {
                        const otherUserId =
                            thread.user_1_id === user.id ? thread.user_2_id : thread.user_1_id;
        ...
        const handleThreadUpdate = async (payload: any) => {
            ...
            const otherUserId =
                updatedThread.user_1_id === user.id ? updatedThread.user_2_id : updatedThread.user_1_id;
            ...
            setHistory((prev) => {
                const existingIndex = prev.findIndex(t => t.thread_id === updatedThread.thread_id);
